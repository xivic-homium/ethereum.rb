...
describe EthereumClient do
  ...
  it "builds, deploys, call methods, receives events and kills contract", blockchain: true do
    ...
    expect(EthereumClient::Transaction.from_blockchain(tx_address).mined?).to be true

    expect(@works.get_filter_logs.changed(filter_id)[0][:transactionHash]).to eq tx_address

    @works_reloaded = EthereumClient::Contract.create(name: "WorksReloaded", address: contract_address, abi: @works.abi, client: client)
